...
export async function updateUser(id, name, email, profilePicture) {
    const profilePic = profilePicture ? profilePicture : 'default-profile.png';
    const res = await conn.query('UPDATE "Users" SET name = $1, email = $2, "profilePicture" = $3 WHERE id = $4 RETURNING *', [name, email, profilePic, id]);
    return res.rows[0];
}